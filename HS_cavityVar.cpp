...
// Function to calculate viscosity based on cell temperature
...
// Function to calculate thermal conductivity based on cell temperature
...
// Function to apply boundary conditions based on the specified case
...
// Function to interpolate boundary conditions for a given side and location
...
// Function to define the left wall velocity profile using an exponential function
...
// Function to define the right wall velocity profile using an exponential function
...
// Function to define the top plate temperature profile using an exponential function
...
// Function to calculate and set up boundary conditions for all walls
...
// Function to initialize all variables, allocate memory, and set initial conditions
void initialization()
{
    h = L/(double)N;
    nsq = N*N;
...
// Function to compute provisional velocities based on current state
...
// Function to calculate the residual for the iterative solver
void residual()
{
    ...
    {
      if(r[i]<0.0){
        if(-1.0*r[i] > infi_norm) 
          infi_norm = -1.0*r[i];
      }
      else if(r[i] > infi_norm) 
        infi_norm = r[i];
    }
}

// Function to compute the right-hand side vector for the pressure equation
double b(int m)
{
...
// Function to perform matrix-vector multiplication for the pressure matrix
...
// Function to compute the dot product of two vectors, excluding a specific grid location
...
// Function to perform pressure iteration using the BiCGStab method
...
// Function to update velocities based on the computed pressure field
void new_velocity()
{
    for(int i=1; i<N; i++)
    {
...
// Function to update the temperature field based on current velocities and thermal properties
...
// Function to compute average velocity values for analysis
void ave_values()
{
    for(int i=0; i<=N; i++)
    {
...
// Function to write simulation results to output files
void write_results()
{
    ofstream print;
...
// Function to calculate shear stress in the flow field
...
// Function to calculate vorticity in the flow field
...
// Function to calculate the residual norm for velocity updates
void UV_residual()
{
    UV_norm = 0.0;
    for(int i=1; i<N; i++)
...
// Function to calculate wall shear stresses and shear force
...
// Function to determine the convergence of velocity fields
...
int main()
{
    ...
    deallocate(LeftBou, N, 3); 
    deallocate(RightBou, N, 3); 
    deallocate(TopBou, N, 3); 
    deallocate(BottomBou, N, 3);
